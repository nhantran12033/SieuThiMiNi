...
class ImportGUI:
    def __init__(self):
        ...
        self.lstProductType = ProductTypeSupplierBiz().get_all_product_type_supplier(
            cond={"supplier_id": self.supplier_id, "is_active": 1}, fields=["product_type_id"])
        ...
        # get list product
        self.lstProduct = ProductBiz().get_all_product(cond={"is_active": 1},
                                                       fields=['id', 'name', 'price', "product_type_id"])
        ...
        # tạo table
        table_purchase_detail = sg.Table(values=[], headings=self.HeadingsPurcharDetail, justification="center",
                                         key='-TABLE_DETAIL_PURCHASE-', enable_events=True)

        table_list_product = sg.Table(values=self.resultPrdctWthTp, headings=self.HeadingsProduct,
                                      justification="center", key='-TABLE_LIST_PRODUCT-', enable_events=True)

        self.spinCount = sg.Spin([i for i in range(10)], initial_value=1, key="-SPIN_COUNT-", enable_events=True,
                                 font="blod")
        ...
        layout3 = [[sg.Text(text='Supplier', font="blod", size=13),
                    sg.Combo(values=self.resultNameSpplr, default_value=self.resultNameSpplr[0], key="-COMBO_SUPPLIER-",
                             enable_events=True)],
                   [sg.Text(text='Product type', font="blod", size=13),
                    sg.Combo(values=self.resultNamePrdctTp, key="-COMBO_PRODUCT_TYPE-", enable_events=True)],
        ...
        while True:
            ...
            if event == "Exit" or event == sg.WINDOW_CLOSED:
                ...
                if len(self.resultPrchslDtl) == 0:
                    ...
                    self.lstProductType = ProductTypeSupplierBiz().get_all_product_type_supplier(
                        cond={"supplier_id": spplr_id, "is_active": 1}, fields=["product_type_id"])
                    self.resultNamePrdctTp = []
                ...
                if selected_row and not selected_row_purchase:
                    ...
                    if count:
                        ...
                        if len(self.resultPrchslDtl) == 0:
                            ...
                            self.window["-SPIN_COUNT-"].Update(value=1,
                                                               values=[i for i in range(-count, 100 - count + 1, 1)])
                            ...
                            if j != -1:
                                ...
                                self.window["-SPIN_COUNT-"].Update(value=0, values=[i for i in
                                                                                    range(-self.resultPrchslDtl[j][2],
                                                                                          100 - self.resultPrchslDtl[j][
                                                                                              2] + 1, 1)])
                            else:
                                item_click = self.resultPrdctWthTp[selected_row[0]].copy()
                                total = float(item_click[2] * count)
                    ...
                    if count:
                        if len(self.resultPrchslDtl) > 0:
                            self.resultPrchslDtl[selected_row_purchase[0]][2] = int(
                                self.resultPrchslDtl[selected_row_purchase[0]][2] + count)
                            self.resultPrchslDtl[selected_row_purchase[0]][4] = \
                            self.resultPrchslDtl[selected_row_purchase[0]][2] * \
                            self.resultPrchslDtl[selected_row_purchase[0]][3]
                ...
                purchase = {'id': id[2:], "supplier_id": supplier_id, 'date': date, 'total_price': total_price,
                            'is_active': isActive}
                ...
                if add != -1:
                    for product in self.resultPrchslDtl:
                        purchase_detail = {'purchase_order_id': id[2:], 'product_id': product[0][2:],
                                           'product_name': product[1], 'count': product[2], 'price': product[3],
                                           'subtotal': product[4], 'is_active': 1}

                        flagIvd = PurchaseOrderDetailsBiz().add_purchase_order_details(data=purchase_detail)
                        if flagIvd == -1:
                            break

                        flagPrdct = ProductBiz().update_payment(count=purchase_detail["count"],
                                                                cond={'id': purchase_detail["product_id"]},
                                                                key="increase")
                        if flagPrdct == -1:
                            break
                    self.reset()
