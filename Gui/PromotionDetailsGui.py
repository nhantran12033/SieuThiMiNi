...
class PromotionDetailsGui:
    def __init__(self, promotion_id, status):
        ...
        layout1 = [
            [sg.Text('Promotion Id:', size=15), sg.Text(text='KM' + promotion_id, size=20, key=self.Headings[0])],
            [sg.Text('Product Id:', size=15),
             sg.Combo(values=self.newResultPrdct, default_value=self.newResultPrdct[0], key=self.Headings[1])],
            [sg.Text('Price:', size=15), sg.Input(size=20, key=self.Headings[2])],
            [sg.Text('Status:', size=15),
             sg.Combo(values=["Hoạt động", "Không hoạt động"], size=20, key=self.Headings[3],
                      default_value="Hoạt động")],
        ...
        table = sg.Table(values=self.result, headings=self.Headings, justification="center", key='-TABLE-',
                         enable_events=True)
        layout2 = [[sg.Text('PROMOTION PRODUCT MANAGEMENT', font="blod", justification="center"),
                    sg.Button(image_filename='Picture/refresh-30.png', key='REFRESH')], [table]]
        layout3 = [[sg.Text('Search with:', size=15),
                    sg.Combo(['promotion_id', 'product_id', 'price', 'is_active'], key='-KEY-'),
            ...
            if event == "Exit" or event == sg.WINDOW_CLOSED:
                ...
                if self.stts != "Áp dụng":
                    ...
                    if values[self.Headings[3]] == "Hoạt động":
                        status = 1
                    data = {'promotion_id': promotion_id[2:], 'product_id': product_id, 'price': price,
                            'is_active': status}
                    print(data)
                    ...
                    if selected_row:
                        ...
                        if product_id_old != id:
                            ...
                            if flag:
                                ...
                                flagUpt = True
                            if not flagUpt:
                                update = PromotionDetailsBiz().update_promotion_detais(promotion=data, cond={
                                    'promotion_id': promotion_id[2:], 'product_id': product_id_old})
                                if update != -1:
                ...
                if self.stts != "Áp dụng":
                    promotion_id = self.window[self.Headings[0]].get()
                    product_id = ProductBiz().get_A_from_B(A="id", nameB="name", valueB=values[self.Headings[1]])
                    result = PromotionDetailsBiz().delete_promotion(promotion_id=promotion_id[2:],
