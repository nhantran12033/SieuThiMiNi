...
class HomeGUI:
    def __init__(self, user):
        ...
        self.lstProduct = ProductBiz().get_all_product(cond={"is_active": 1},
                                                       fields=['id', 'name', 'count', 'price', 'discount',
                                                               "product_type_id"])
        ...
        # tạo table
        table_detail_invoices = sg.Table(values=[], headings=self.HeadingsDetailInvoices, justification="center",
                                         key='-TABLE_DETAIL_INVOICES-', enable_events=True)
        ...
        menu = [['Management',
                 ['Account', 'Staff', 'Supplier', 'Product type', 'Product', 'Membership', 'Coupou', 'Promotion',
                  'Import', ]], ['Statiѕtic', ['Statiѕtic']], ['Account', ['Change Password', 'Log out']], ]
        ...
        layout3 = [[sg.Text(text='Product type', font="blod", size=13),
        ...
        while True:
            ...
            # onCLick element of table
                ...
                if selected_row and not selected_row_invoices:
                    ...
                    if count:
                        if len(self.resultDtlInvc) > 0:
                            self.resultDtlInvc[selected_row_invoices[0]][2] = int(
                                self.resultDtlInvc[selected_row_invoices[0]][2] + count)
                            self.resultDtlInvc[selected_row_invoices[0]][6] = \
                            self.resultDtlInvc[selected_row_invoices[0]][2] * \
                            self.resultDtlInvc[selected_row_invoices[0]][5]
                ...
                sg.popup("Coupou Discount:{} VNĐ\nPoint Discount:{} VNĐ\nTotal Discount: {} VNĐ".format(
                    float(self.discount_cp), float(self.discount_pnt), float(self.discount)))
                            ...
                            if event_input in (sg.WIN_CLOSED, 'Cancel'):
                                ...
                                if values_input["-COMFIRM-"] == "":
                                    ...
                                    if values_input["-COMFIRM-"] == v_c:
                                        ...
                                        sg.popup("Success")
                                        self.window["-SPIN_POINT-"].Update(
                                            values=[i for i in range(0, self.point + 1, 1000)])
                ...
                invoices = {'id': id[2:], 'date': date, 'staff_id': staff_id, 'membership_id': membership_id,
                            'total_price': total_price, 'discount': discount, 'remaining_price': remaining_price,
                            'coupou_id': coupou_id, 'point': point, 'is_active': isActive}
                ...
                if add != -1:
                    for product in self.resultDtlInvc:
                        invoices_detail = {'invoice_id': id[2:], 'product_id': product[0][2:],
                                           'product_name': product[1], 'count': product[2], 'price': product[5],
                                           'subtotal': product[6], 'is_active': 1}

                        flagIvd = InvoiceDetailsBiz().add_invoice_details(data=invoices_detail)
                        if flagIvd == -1:
                            break

                        flagPrdct = ProductBiz().update_payment(count=invoices_detail["count"],
                                                                cond={'id': invoices_detail["product_id"]},
                                                                key="decrease")
                        if flagPrdct == -1:
                            break
                    if membership_id:
                        point_update = self.point - self.discount_pnt + point
                        flagMbs = MembershipsBiz().update_memberships(memberships={'point': point_update},
                                                                      cond={"id": membership_id})
                        if flagMbs == -1:
                            break
                    self.reset()
