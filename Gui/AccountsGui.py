    ...
    def __init__(self):
        ...
        layout2 = [[sg.Text('DANH SÁCH TÀI KHOẢN', font="blod", size=70, justification="center")],
                   [sg.Table(values=self.result, headings=self.Headings, justification="center", key='-TABLE-',
                             enable_events=True)]]
        layout1 = [[sg.Text('Chọn từ khóa search:', size=15),
                    sg.Combo(['id', 'username', 'password', 'role_id', 'status', "is_active"], default_value="id",
                             key='-COMBO_SEARCH-', enable_events=True), sg.Text('Content:'),
                   ...
                   [sg.Text('IsActive:', size=15),
                    sg.Combo(key=self.Headings[5], values=["Hoạt động", "Không hoạt động"], default_value="Hoạt động")],
    ...
    def run(self):
        ...
        while True:
            ...
            if event == "Exit" or event == sg.WINDOW_CLOSED:
                ...
                account = {"id": id[2:], 'username': username, 'password': password, 'role_id': role_id,
                           'status': status, 'is_active': 1}
                ...
                data = {"id": id[2:], 'username': username, 'password': password, 'role_id': role_id, 'status': status,
                        'is_active': is_active}
