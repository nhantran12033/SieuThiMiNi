...
class SpplrPrdctTpGui:
    def __init__(self, supplier_id):
        ...
        table = sg.Table(values=self.result, headings=self.Headings, justification="center", key='-TABLE-',
                         enable_events=True)
        layout2 = [[sg.Text('SUPPLIER PRODUCT TYPE MANAGEMENT', font="blod", justification="center"),
                    sg.Button(image_filename='Picture/refresh-30.png', key='REFRESH')], [table]]
        layout3 = [
            [sg.Text('Search with:', size=15), sg.Combo(['supplier_id', 'product_type_id', 'status'], key='-KEY-'),
        ...
        while True:
            ...
            if event == "Exit" or event == sg.WINDOW_CLOSED:
                ...
                if selected_row:
                    ...
                    product_type_id_old = ProductTypesBiz().get_A_from_B(A="id", nameB="name",
                                                                         valueB=product_type_name_old)
                    ...
                    if product_type_id_old != id:
                        ...
                        if flag:
                            ...
                            flagUpt = True
                        if not flagUpt:
                            update = SpplrPrdctTpBiz().update_spplrPrdctTp(data=data,
                                                                           cond={'supplier_id': supplier_id[2:],
                                                                                 'product_type_id': product_type_id_old})
                            if update != -1:
