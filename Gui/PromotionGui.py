...
class PromotionsGUI:
    def __init__(self):
        ...
        layout1 = [[sg.Text('Id:', size=15), sg.Button("ID", key="NEWID"), sg.Text(size=20, key=self.Headings[0])],
                   ...
                   [sg.Text('Date from:', size=15),
                    sg.CalendarButton('', image_filename='Picture/calendar-24.png', format='%Y-%m-%d',
                                      target=self.Headings[2], size=22), sg.Input(size=20, key=self.Headings[2])],
                   [sg.Text('Date to:', size=15),
                    sg.CalendarButton('', image_filename='Picture/calendar-24.png', format='%Y-%m-%d',
                                      target=self.Headings[3], size=22), sg.Input(size=20, key=self.Headings[3])],
                   [sg.Text('Status:', size=15), sg.Text(text='Không áp dụng', key=self.Headings[4])],
                   [sg.Text('IsActive:', size=15),
                    sg.Combo(['Hoạt động', 'Không hoạt động'], default_value='Hoạt động', key=self.Headings[5])],
                   [sg.Button('Thêm'), sg.Button('Sửa'), sg.Button('Chi tiết'), sg.Button('Xóa'), sg.Button("Áp dụng"),
                    sg.Button("Ngưng áp dụng")],
                   ]
        # tạo table
        table = sg.Table(values=self.result, headings=self.Headings, justification="center", key='-TABLE-',
                         enable_events=True)
        layout2 = [[sg.Text('PROMOTIONS MANAGEMENT', font="blod", justification="center"),
                    sg.Button(image_filename='Picture/refresh-30.png', key='REFRESH')], [table]]
        layout3 = [[sg.Text('Search with:', size=15),
                    sg.Combo(['id', 'promotion_name', 'date_from', 'date_to', 'status', 'isActive'], key='-KEY-'),
    ...
    def run(self):
        ...
        while True:
            ...
            if event == "Exit" or event == sg.WINDOW_CLOSED:
                ...
                add = PromotionsBiz().add_promotion(
                    {'id': id[2:], 'promotion_name': promotion_name, 'date_from': date_from, 'date_to': date_to,
                ...
                if self.window[self.Headings[4]].get() == "Áp dụng":
                    ...
                    promotion = {'promotion_name': promotion_name, 'date_from': date_from, 'date_to': date_to,
                                 'status': status, 'is_active': is_active}
                        ...
                        for item in self.result:
                            if item[0] == id:
                                if item[2] < today and today < item[3]:
                                    flag = True
                        if flag:
                            dulieu = PromotionDetailsBiz().get_all_promotion_details(
                                cond={"promotion_id": id[2:], "is_active": 1})
                            result = []
                            for item in dulieu:
                                item = list(item)
                                result.append(item)
                            ...
                            if upd == -1:
                                ...
                                isErr = False
                                for item in result:
                                    updPrdct = ProductBiz().update_product(product={"discount": item[2]},
                                                                           cond={"id": item[1]})
                                    if updPrdct == -1:
                                        isErr = True
                                if not isErr:
                    ...
                    if selected_row:
                        if self.result[selected_row[0]][4] == "Áp dụng":
                            ...
                            if upd == -1:
                                ...
                                isErr = False
                                for item in result:
                                    updPrdct = ProductBiz().update_product(product={"discount": 0},
                                                                           cond={"id": item[1]})
                                    if updPrdct == -1:
                                        isErr = True
                                if not isErr:
