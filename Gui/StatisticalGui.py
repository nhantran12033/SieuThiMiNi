...
class Statistical:
    def __init__(self):
        ...
        self.infoAccount = 'Số tài khoản được cấp: {}\nSố tài khoản hoạt động: {}\nSố tài khoản không hoạt động:{}'.format(
            self.ttAccount["cap"], self.ttAccount["hoatdong"], self.ttAccount["kohoatdong"])

        self.ttStaff = StaffsBiz().get_info_staff()
        self.infoStaff = 'Tổng số nhân viên hoạt động: {}\nTổng số nhân viên thôi làm:{}'.format(
            self.ttStaff["hoatdong"], self.ttStaff["kohoatdong"])

        self.ttPrdctTp = ProductTypesBiz().get_info_prdctTp()
        self.infoPrdctTp = 'Tổng số loại sản phẩm tồn tại: {}\nTổng số loại sản phẩm không tồn tại:{}'.format(
            self.ttPrdctTp["hoatdong"], self.ttPrdctTp["kohoatdong"])

        self.ttPrdct = ProductBiz().get_info_prdct()
        self.infoPrdct = 'Tổng số sản phẩm tồn tại: {}\nTổng số sản phẩm không tồn tại:{}'.format(
            self.ttPrdct["hoatdong"], self.ttPrdct["kohoatdong"])

        self.ttSpplr = SuppliersBiz().get_info_spplr()
        self.infoSpplr = 'Tổng số nhân viên hoạt động: {}\nTổng số nhân viên thôi làm:{}'.format(
            self.ttSpplr["hoatdong"], self.ttSpplr["kohoatdong"])

        self.ttPromotion = PromotionsBiz().get_info_promotion()
        self.infoPromotion = 'Tổng số ct khuyến mãi đã tạo: {}\nTổng số ct khuyến mãi đã xóa:{}\nCT Khuyến mãi đang áp dụng là: {}'.format(
            self.ttPromotion["hoatdong"], self.ttPromotion["kohoatdong"], self.ttPromotion["apdung"])

        self.ttCoupou = CoupousBiz().get_info_coupou()
        self.infoCoupou = 'Tổng số voucher đã tồn tại: {}\nTổng số voucher đã đã xóa:{}\nTổng số voucher đã chưa sử dụng: {}\nTổng số voucher đã sử dụng: {}'.format(
            self.ttCoupou["hoatdong"], self.ttCoupou["kohoatdong"], self.ttCoupou["sudung"],
            self.ttCoupou["chuasudung"])

        self.ttMem = MembershipsBiz().get_info_mem()
        self.infoMem = 'Tổng số thành viên tồn tại: {}\nSố thành viên ngừng sử dụng:{}'.format(self.ttMem["hoatdong"],
                                                                                               self.ttMem["kohoatdong"])

        # TABLAYOUT Tổng kết
        self.key_statistical = ['accounts', 'staff', 'memberships', 'product_types', 'products', 'suppliers', 'coupous',
                                'promotions']
        layout_00 = [[sg.Text('Tài khoản', font=17)], [
            sg.Multiline(disabled=True, size=(50, 40), default_text=self.infoAccount, justification='center', font=13,
                         key=self.key_statistical[0])]]
        layout_01 = [[sg.Text('Nhân viên', font=17)], [
            sg.Multiline(disabled=True, size=(50, 40), default_text=self.infoStaff, justification='center', font=13,
                         key=self.key_statistical[1])]]
        layout_02 = [[sg.Text('Thành viên', font=17)], [
            sg.Multiline(disabled=True, size=(50, 40), default_text=self.infoMem, justification='center', font=13,
                         key=self.key_statistical[2])]]
        layout_10 = [[sg.Text('Loại Sản Phẩm', font=17)], [
            sg.Multiline(disabled=True, size=(50, 40), default_text=self.infoPrdctTp, justification='center', font=13,
                         key=self.key_statistical[3])]]
        layout_11 = [[sg.Text('Sản phẩm', font=17)], [
            sg.Multiline(disabled=True, size=(50, 40), default_text=self.infoPrdct, justification='center', font=13,
                         key=self.key_statistical[4])]]
        layout_12 = [[sg.Text('Nhà cung cấp', font=17)], [
            sg.Multiline(disabled=True, size=(50, 40), default_text=self.infoSpplr, justification='center', font=13,
                         key=self.key_statistical[5])]]
        layout_20 = [[sg.Text('Phiếu giảm giá', font=17)], [
            sg.Multiline(disabled=True, size=(50, 40), default_text=self.infoCoupou, justification='center', font=13,
                         key=self.key_statistical[6])]]
        layout_21 = [[sg.Text('Chương trình khuyến mãi', font=17)], [
            sg.Multiline(disabled=True, size=(50, 40), default_text=self.infoPromotion, justification='center', font=13,
                         key=self.key_statistical[7])]]
        frame_size = (
        int(default_with / 3), int(default_height / 3))  # đặt kích thước chung trừ chương trình khuyến mãi
        # Tạo layout Tổng quát
        grid_layout = [
            [sg.Frame('', layout_00, size=frame_size), sg.Frame('', layout_01, size=frame_size),
             sg.Frame('', layout_02, size=frame_size)],
            [sg.Frame('', layout_10, size=frame_size), sg.Frame('', layout_11, size=frame_size),
             sg.Frame('', layout_12, size=frame_size)],
            [sg.Frame('', layout_20, size=frame_size),
             sg.Frame('', layout_21, size=(int(default_with * 2 / 3), int(default_height / 5)))]
        ]
        # Giao diện tab Hóa đơn
        frame_size_order = (int(default_with / 4), int(default_height / 8))
        top_01 = [[sg.Input('', key='DATE_FROM_ORDER', size=12, justification='center', enable_events=True),
                   sg.CalendarButton('', image_filename='Picture/calendar-24.png', format='%Y-%m-%d',
                                     target='DATE_FROM_ORDER', size=(24, 24))]]
        top_02 = [[sg.Combo(['id', "staff_name", "membership_name"], key='TYPE_SEARCH')]]
        top_03 = [[sg.Input('', key='CONTENT_SEARCH', enable_events=True)]]
        top_04 = [[sg.Input('', key='DATE_TO_ORDER', size=12, enable_events=True),
                   sg.CalendarButton('', image_filename='Picture/calendar-24.png', format='%Y-%m-%d',
                                     target='DATE_TO_ORDER', size=(24, 24))]]
        ...
        # Tạo table
        self.key_statistical_order = ['Id Invoice', 'Date', 'Staff', 'Membership', 'Total Price ', 'Discount',
                                      'Remain Price', 'Coupou', 'Point']
        ...
        table_order = [[sg.Table(values=self.data_order, headings=self.key_statistical_order, justification="center",
                                 key='-TABLE_ORDER-', enable_events=True, vertical_scroll_only=False)]]

        # bottom  order
        bottom_01 = [[sg.Text('Giá trị lớn nhất của hóa đơn trong danh sách')],
                     [sg.Text(text=InvoicesBiz().get_invoice_max_price(), size=10, justification='center',
                              key='ORDER_MAX')]]
        bottom_02 = [[sg.Text('Giá trị nhỏ nhất của hóa đơn trong danh sách')],
                     [sg.Text(text=InvoicesBiz().get_invoice_min_price(), size=10, justification='center',
                              key='ORDER_MIN')]]
        bottom_03 = [[sg.Text('Doanh thu năm hiện tại')],
                     [sg.Text(text=InvoicesBiz().get_invoice_by_year(), size=10, justification='center',
                              key='REVENUE_YEAR')]]
        bottom_04 = [[sg.Text('Doanh thu tháng hiện tại ')],
                     [sg.Text(text=InvoicesBiz().get_invoice_by_month(), size=10, justification='center',
                              key='REVENUE_MONTH')]]
        bottom_05 = [[sg.Text('Doanh thu ngày hiện tại')],
                     [sg.Text(text=InvoicesBiz().get_invoice_by_day(), size=10, justification='center',
                              key='REVENUE_DAY')]]
        bottom_size = (int(default_with / 5), int(default_height / 10))
        # main thống kê hóa đơn
        statistical_order = [[sg.Frame('Từ ngày', top_01, size=frame_size_order, element_justification='center'),
                              sg.Frame('Thống kê theo', top_02, size=(int(default_with / 8), int(default_height / 8)),
                                       element_justification='center'),
                              sg.Frame('Tìm kiếm ', top_03, size=frame_size_order, element_justification='center'),
                              sg.Frame('Đến ngày', top_04, size=frame_size_order, element_justification='center')],
                             [sg.Frame('', top_11, size=frame_size_order, element_justification='center',
                                       border_width=0),
                              sg.Frame('Thống kê theo', top_12, size=(int(default_with / 8), int(default_height / 8)),
                                       element_justification='center'),
                              sg.Frame('Từ', top_13, size=(int(default_with / 8), int(default_height / 8)),
                                       element_justification='center'),
                              sg.Frame('Đến', top_14, size=(int(default_with / 8), int(default_height / 8)),
                                       element_justification='center'),
                              sg.Frame('', top_15, size=frame_size_order, element_justification='center',
                                       border_width=0)],
                             [sg.Frame('', table_order, size=(int(default_with), int(default_height / 3)))],
                             [sg.Frame('', bottom_01, size=(int(default_with / 6.5), int(default_height / 10))),
                              sg.Frame('', bottom_02, size=(int(default_with / 6.5), int(default_height / 10))),
                              sg.Frame('', bottom_03, size=bottom_size),
        ...
        table_entry = [[sg.Table(values=self.data_purchase, headings=self.key_statistical_entry, justification="center",
        ...
        bottom_entry_01 = [[sg.Text('Chi phí đơn hàng lớn nhất trong danh sách')],
                           [sg.Text(text=PurchaseOrdersBiz().get_purchase_max_price(), size=10, justification='center',
                                    key='ENTRY_MAX')]]
        bottom_entry_02 = [[sg.Text('Chi phí đơn hàng nhỏ nhất trong danh sách')],
                           [sg.Text(text=PurchaseOrdersBiz().get_purchase_min_price(), size=10, justification='center',
                                    key='ENTRY_MIN')]]
        bottom_entry_03 = [[sg.Text('Chí phí nhập năm hiện tại')],
                           [sg.Text(text=PurchaseOrdersBiz().get_purchase_by_year(), size=10, justification='center',
                                    key='ENTRY_YEAR')]]
        bottom_entry_04 = [[sg.Text('Chi phí nhập tháng hiện tại ')],
                           [sg.Text(text=PurchaseOrdersBiz().get_purchase_by_month(), size=10, justification='center',
                                    key='ENTRY_MONTH')]]
        bottom_entry_05 = [[sg.Text('Chi phí nhập ngày hiện tại')],
                           [sg.Text(text=PurchaseOrdersBiz().get_purchase_by_day(), size=10, justification='center',
                                    key='ENTRY_DAY')]]
        ...
        statistical_entry = [[sg.Frame('Từ ngày', top_entry_01, size=frame_size_entry, element_justification='center'),
                              sg.Frame('Thống kê theo', top_entry_02,
                                       size=(int(default_with / 8), int(default_height / 8)),
                                       element_justification='center'),
                              sg.Frame('Tìm kiếm ', top_entry_03, size=frame_size_entry,
                                       element_justification='center'),
                              sg.Frame('Đến ngày', top_entry_04, size=frame_size_entry,
                                       element_justification='center')],
                             [sg.Frame('', top_entry_11, size=frame_size_entry, element_justification='center',
                                       border_width=0),
                              sg.Frame('Tổng tiền từ', top_entry_13,
                                       size=(int(default_with / 8), int(default_height / 8)),
                                       element_justification='center'),
                             ...
                             [sg.Frame('', bottom_entry_01,
                                       size=(int(default_with / 6.6 + 2), int(default_height / 10))),
                              sg.Frame('', bottom_entry_02,
                                       size=(int(default_with / 6.6 + 2), int(default_height / 10))),
                              sg.Frame('', bottom_entry_03, size=bottom_entry_size),
                              sg.Frame('', bottom_entry_04, size=bottom_entry_size),
                              sg.Frame('', bottom_entry_05, size=bottom_entry_size)]
                             ]
        # Tạo TabGroup
        tabgroup_layout = [
            ...
            if event == sg.WINDOW_CLOSED:
                ...
                if value_from and value_to:
                    ...
                    for idx in range(len(Entitys)):
                        if int(value_from) < int(getattr(Entitys[idx], search_with)) and int(
                                getattr(Entitys[idx], search_with)) < int(value_to):
                ...
                if value_from and value_to:
                    ...
                    result = []
                    for idx in range(len(Entitys)):
                        if int(value_from) < int(getattr(Entitys[idx], 'total_price')) and int(
                                getattr(Entitys[idx], 'total_price')) < int(value_to):
