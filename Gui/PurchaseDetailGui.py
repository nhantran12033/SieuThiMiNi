...
class PurchaseDetailsGui:
    def __init__(self, purchase_id):
        ...
        self.lst = PurchaseOrderDetailsBiz().get_all_purchase_order_details(
            cond={"purchase_order_id": purchase_id, "is_active": 1})
        self.result = []
        ...
        table = sg.Table(values=self.result, headings=self.Headings, justification="center", key='-TABLE-',
                         enable_events=True)
        layout2 = [[sg.Text('PURCHASE ORDER DETAIL MANAGEMENT', font="blod", justification="center"),
                    sg.Button(image_filename='Picture/refresh-30.png', key='REFRESH')], [table]]
        layout3 = [[sg.Text('Search with:', size=15),
                    sg.Combo(['purchase_order_id', 'product_id', 'product_name', 'count', 'price', 'total', 'status'],
                             key='-KEY-'),
    ...
    def run(self):
        ...
        while True:
            ...
            if event == "Exit" or event == sg.WINDOW_CLOSED:
                ...
                self.lst = PurchaseOrderDetailsBiz().get_all_purchase_order_details(
                    cond={"purchase_order_id": self.id, "is_active": 1})
                self.result = []
                # for i in self.dulieu:
                #     self.result.append(list(i))
                for item in self.lst:
